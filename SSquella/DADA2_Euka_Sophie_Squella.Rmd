---
title: "DADA2_Euka_Sophie_Squella"
date: 22-03-2022
output: html_document
---

## Bioblioteca de funciones
...
Ojo que modifique (acorte) el nombre de entrada del archivo sólo para simplificar la lectura, se mantuvo el mismo orden
...
En general se aprecia una buena calidad de secuencias salvo la muestra "LOM_78", aún así procederemos de acuerdo a lo evienciado por la mayoria y aquellas muestras con más lecturas
...
taxa <- assignTaxonomy(seqtab.nochim, "pr2_version_4.14.0_SSU_dada2.fasta.gz", taxLevels = c("Kingdom", "Supergroup", "Division", "Class", "Order", "Family", "Genus", "Species"), multithread = TRUE)
```
...
write.csv2(taxa, file = "taxa_euk.csv")
```
...
write(asv_fasta, "seqs_euka_lom.fasta")

#count table
asv_tab <- t(seqtab.nochim) %>% as.data.frame()
rownames(asv_tab) <- sub(">", "", asv_headers)
colnames(asv_tab) <- sample.names
asv_tab <- mutate(asv_tab, Seqs = all_of(colnames(seqtab.nochim)), .before = "LOM_70")
